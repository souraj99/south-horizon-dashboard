...
const EditCoupon: React.FC = () => {
  ...
  useEffect(() => {
    const couponData = localStorage.getItem("editCouponData");
    if (couponData) {
      try {
        const parsedData = JSON.parse(couponData);

        // Format date for input field (YYYY-MM-DD)
        let formattedDate = "";
        if (parsedData.expiresAt) {
          const date = new Date(parsedData.expiresAt);
          formattedDate = date.toISOString().split("T")[0];
        }

        // Set form values
        setValue("code", parsedData.code || "");
        setValue("discount", parsedData.discount || 0);
        setValue("expiresAt", formattedDate);
        setValue("maxUses", parsedData.maxUses || 1);
        setValue("minOrderAmount", parsedData.minOrderAmount || 0);
        setValue("description", parsedData.description || "");
        setValue(
          "isActive",
          parsedData.isActive !== undefined ? parsedData.isActive : true
        );

        // Set coupon ID for update operations
        setCouponId(parsedData.id || "");

        // Clear localStorage after loading
        localStorage.removeItem("editCouponData");
      } catch (error) {
        console.error("Error parsing coupon data:", error);
        setSubmitMessage({
          type: "error",
          message: "Error loading coupon data. Please try again.",
        });
      }
    }
  }, [setValue]);
  ...
  const onSubmit = async (data: CouponFormData) => {
    ...
    try {
      ...
      // Navigate back to coupons page after successful update
      setTimeout(() => {
